...
protocol GitHubSessionType: AnyObject {
...
extension GitHub.Session: GitHubSessionType {
    ...
    func send<T: GitHubRequest>(_ request: T) -> Observable<Response<T>> {
        ...
        guard var componets = URLComponents(url: url, resolvingAgainstBaseURL: true) else { return Observable<Response<T>>.just(.right(SessionError.failedToCreateComponents(url))) }
        
        componets.queryItems = request.queryParameters?.compactMap(URLQueryItem.init)

        guard var urlRequest = componets.url.map({ URLRequest(url: $0) }) else { return Observable<Response<T>>.just(.right(SessionError.failedToCreateURL(componets))) }
